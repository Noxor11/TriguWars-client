...
void TitleScreen::update(float dt) {
    ...
    const float message_y = TOP_HEIGHT_CENTER + graphics::text::pt_to_px(30);
    #elif defined(__PSVITA__) || defined(__PC__)
    const float title_x = WIDTH_CENTER;
    const float title_y = HEIGHT_CENTER;

    const float message_x = title_x;
    const float message_y = HEIGHT_CENTER + graphics::text::pt_to_px(30);
    #endif

    graphics::text::draw_text(this->vscreen.translate_x(title_x), this->vscreen.translate_y(title_y), "TriguWars", graphics::text::pt_to_size(30), true);
    graphics::text::draw_text(this->vscreen.translate_x(message_x), this->vscreen.translate_y(message_y),"Press X or A to continue", graphics::text::pt_to_size(30), true);
